...
if __name__ == '__main__':
    ...
    OUTPUT_DIR = ROOT / "detections" / "bcnet"
    STYLE_VERSIONS = ["1"]

    MODELS = [
        ["fcos_imprv_R_101_FPN", "configs/fcos/fcos_imprv_R_101_FPN.yaml", "model_final_beta.pth"],
    ]

    CUDA = 'CUDA_VISIBLE_DEVICES="0,1,2,3"'
    # switch to bmask repository
    BC_ROOT = ROOT / 'ext/BCNet'
    os.chdir(BC_ROOT)
    ...
    for model, cfg, weights in MODELS:
        ...
        os.system(f"{CUDA} python tools/train_net.py --config-file {cfg} --eval-only --num-gpus 4 MODEL.WEIGHTS ./{weights}")
        eval_and_save(f'./output/inference/coco_instances_results.json', output_dir, ANNOTATIONS)
